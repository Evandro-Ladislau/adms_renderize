...
if (!empty($id)) {
    ...
    if ($result_niv_ac_pg) {
        ...
        if ($vetor_result_niv_ac_pg['dropdown'] == 1) {
            $status = 2;
        } else {
            $status = 1;
        }

        $result_niv_pg_update = $pdo->atualizarLiberMenuDropdown($status, $id);
        ...
        if ($result_niv_pg_update) {
            ...
            $_SESSION['msg'] = "<div class='alert alert-danger'> Erro: Menu dropdown não foi alterada! </div>";
            $url_destino = pg . '/listar/list_permissao?id='. $vetor_result_niv_ac_pg['adms_niveis_acesso_id'];
            header("Location: $url_destino");
        }
    } else {
